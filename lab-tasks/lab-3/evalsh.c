  ...
  //               }
	 ...
	 //   		strcpy(buf, redr[0]);
		...
		else if(strcmp(token[0], "echo") == 0)
		{
			...
			{
				char *ecpath = "/bin/echo";
				char *myargs[sizeof(buf)];

				myargs[1] = NULL;
				myargs[0] = strdup(ecpath);
				...
				if(strcmp(token[1], "<") == 0)
				{
					...
					read(fdi, input, sizeof(input));
					myargs[1] = strdup(input);
				}
